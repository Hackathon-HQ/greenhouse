...
function renderStreamEvent(raw: string): {
  ...
  switch (ev?.type) {
    ...
    // Top-level reasoning stream — the model thinking out loud between actions.
    // This fills the gap (e.g. init -> first edit) with live interim info.
    case "thinking": {
      const t = String(ev.text ?? ev.thinking ?? "").replace(/\s+/g, " ").trim();
      return { line: t.length > 2 ? `thinking: ${t.slice(0, 400)}` : null };
    }
    case "result": {
...
/**
 * Write `cited.md` — the real evidence the idea was built on (the source URLs +
 * verbatim quotes). Shipped with the MVP so the build is traceable to demand.
 */
async function writeCitedMd(idea: AppIdea, dir: string): Promise<void> {
  const lines = [
    `# ${idea.title} — Citations`,
    ``,
    `> ${idea.pitch}`,
    ``,
    `**Problem.** ${idea.problem}`,
    ``,
    `## Evidence (where the demand was found)`,
    ``,
  ];
  if (idea.evidence?.length) {
    for (const e of idea.evidence) {
      lines.push(`- ${e.quote ? `“${e.quote}”` : "(source)"}`);
      lines.push(`  — [${e.source}](${e.url})`);
      lines.push("");
    }
  } else if (idea.sourceQuote) {
    lines.push(`- “${idea.sourceQuote}”`, "");
  } else {
    lines.push("_No citations recorded for this idea._", "");
  }
  lines.push(`---`, `_Generated by AppTok for idea \`${idea.id}\`._`);
  await fs.writeFile(path.join(dir, "cited.md"), lines.join("\n"), "utf8");
}
...
export async function buildIdea(
  ...
  try {
    ...
    // Publish cited.md — the real evidence behind the idea, shipped with the MVP.
    pushLog(artifact, log, "[build] publishing cited.md");
    await writeCitedMd(idea, dir);
    producedFiles = await walkFiles(dir);
